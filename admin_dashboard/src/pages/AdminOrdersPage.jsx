    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className={'mt-4 flex flex-wrap gap-3 ' + card}>
          <select value={orderStatus} onChange={(e) => { setOrderStatus(e.target.value); setPage(1) }} className="rounded-lg border border-slate-300 px-3 py-1.5 text-sm dark:border-slate-700 dark:bg-slate-800">
            {ORDER_STATUS.map((s) => <option key={s} value={s}>{s || 'All order statuses'}</option>)}
          </select>
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead><tr className="border-b border-slate-200 dark:border-slate-700">
                  <th className="pb-2 pr-3">Order #</th><th className="pb-2 pr-3">User</th><th className="pb-2 pr-3">Warehouse</th><th className="pb-2 pr-3">Total</th><th className="pb-2 pr-3">Status</th><th className="pb-2 pr-3">Payment</th><th className="pb-2">Date</th>
                </tr></thead>
                <tbody>
                  {orders.map((o) => (
                    <tr key={o.order_id} className="border-b border-slate-100 dark:border-slate-800">
                      <td className="py-2 pr-3"><Link to={`/admin/orders/${o.order_id}`} className="text-emerald-600 hover:underline dark:text-emerald-400">{o.order_number || o.order_id.slice(0, 8)}</Link></td>
                      <td className="py-2 pr-3">{o.user?.full_name || '--'}</td>
                      <td className="py-2 pr-3">{o.warehouse?.name || '--'} ({o.warehouse?.city || ''})</td>
                      <td className="py-2 pr-3">₹{o.total}</td>
                      <td className="py-2 pr-3"><span className={badge('bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300')}>{o.order_status}</span></td>
                      <td className="py-2 pr-3"><span className={badge('bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300')}>{o.payment_status}</span></td>
                      <td className="py-2">{new Date(o.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                  {orders.length === 0 && <tr><td colSpan={7} className="py-6 text-center text-slate-500">No orders found</td></tr>}
                </tbody>
              </table>
            </div>
