      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <main className="mt-5 grid gap-4 lg:grid-cols-2">
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-600 dark:text-emerald-300">Vendor Login</p>
            <h1 className="mt-2 text-2xl font-bold tracking-tight">Sign in to continue</h1>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
              Access warehouses, inventory, orders, and fulfillment workflows.
            </p>
            ...
            <form className="mt-5 grid gap-4" onSubmit={handleSubmit} noValidate>
              <div>
                ...
                <input id="email" type="email" value={form.email} onChange={handleChange('email')} className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800" placeholder="you@example.com" autoComplete="email" />
            ...
            <div className="mt-4 text-sm">
              <Link to="/auth/vendor/signup" className="text-emerald-600 hover:text-emerald-500 dark:text-emerald-300">Create vendor account</Link>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <ol className="mt-3 list-inside list-decimal space-y-2 text-sm text-slate-600 dark:text-slate-300">
              <li>Frontend calls <span className="font-semibold">POST /vendor/auth/signin</span>.</li>
              <li>Backend validates vendor role and returns access token.</li>
              <li>Frontend verifies session via <span className="font-semibold">GET /vendor/warehouses</span>.</li>
              <li>Vendor is redirected to dashboard.</li>
            </ol>
          </section>
        </main>
      </div>
