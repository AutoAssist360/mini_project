...
import { ApiError, vendorSignUp } from '../lib/api'
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate('/auth/vendor/signin', {
        replace: true,
        state: { signupSuccess: 'Vendor account created successfully. Please sign in.' },
      })
    } catch (error) {
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <main className="mt-5 grid gap-4 lg:grid-cols-2">
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-emerald-600 dark:text-emerald-300">Vendor Registration</p>
            <h1 className="mt-2 text-2xl font-bold tracking-tight">Create your vendor account</h1>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Create account first, then sign in to access vendor operations.</p>
            ...
            <div className="mt-4 text-sm">
              <Link to="/auth/vendor/signin" className="text-emerald-600 hover:text-emerald-500 dark:text-emerald-300">Already have an account? Sign in</Link>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <ol className="mt-3 list-inside list-decimal space-y-2 text-sm text-slate-600 dark:text-slate-300">
              <li>POST /vendor/auth/signup creates vendor user account.</li>
              <li>Backend validates payload and persists in DB.</li>
              <li>Frontend redirects to vendor sign-in page.</li>
              <li>Vendor signs in manually and enters dashboard.</li>
            </ol>
          </section>
        </main>
      </div>
